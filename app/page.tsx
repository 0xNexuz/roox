...
import { ArrowDownRight, ArrowRight, Box, Check, CircleGauge, Clock3, Cpu, ExternalLink, Globe2, Search, ShieldCheck, Sparkles, Wallet, Zap } from 'lucide-react';
...
export default function Home() {
  ...
  async function connect() {
    ...
    } catch { setNote('Connection cancelled. Your wallet was not changed.'); }
  }

  function runCall() {
    if (run !== 'idle') return;
    setRun('running');
    window.setTimeout(() => setRun('done'), 1500);
  }
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-background text-foreground">
      <header className="fixed inset-x-0 top-0 z-50 border-b border-white/[.07] bg-[#080908]/75 backdrop-blur-xl">
        <div className="mx-auto flex h-16 max-w-[1480px] items-center justify-between px-5 lg:px-10">
          ...
          <div className="flex items-center gap-2">
            <span className="hidden items-center gap-2 text-[10px] uppercase tracking-[.12em] text-white/45 sm:flex"><span className="size-1.5 animate-pulse rounded-full bg-[#c8ff30]" />Robinhood Chain</span>
      ...
      <section id="top" className="border-b border-white/[.08] pt-16">
        <div ref={hero} onPointerMove={moveHero} onPointerLeave={() => { hero.current?.style.setProperty('--pointer-x', '0px'); hero.current?.style.setProperty('--pointer-y', '0px'); }} className="hero-stage relative min-h-[694px] overflow-hidden">
          ...
          <div className="relative z-10 mx-auto flex min-h-[694px] max-w-[1480px] flex-col px-5 pb-8 pt-16 lg:px-10 lg:pt-20">
            ...
            <div className="mx-auto mt-10 max-w-5xl text-center lg:mt-6">
              ...
              <p className="mx-auto mt-6 max-w-xl text-balance text-sm leading-6 text-white/58 sm:text-base">Rent verified GPU capacity for AI inference. Pay per completed call. Settle instantly on Robinhood Chain.</p>
              <div className="mt-7 flex flex-wrap justify-center gap-3">
                <Button render={<a href="#market" />} className="h-11 rounded-full bg-[#c8ff30] px-5 text-sm font-semibold text-black shadow-[0_0_44px_rgba(200,255,48,.18)] hover:bg-[#d6ff61]">Explore compute <ArrowDownRight data-icon="inline-end" /></Button>
                <Button render={<a href="#providers" />} variant="outline" className="h-11 rounded-full border-white/18 bg-black/25 px-5 text-sm text-white backdrop-blur-md hover:bg-white/10">List your GPU <ArrowRight data-icon="inline-end" /></Button>
              </div>
            ...
            <div className="mt-auto grid grid-cols-2 border-y border-white/10 bg-black/20 backdrop-blur-sm sm:grid-cols-4">
              {[['1,284', 'GPUs online'], ['42.8M', 'calls settled'], ['196 ms', 'median latency'], ['$0.0032', 'avg. cost / call']].map(([value, label], i) => <div key={label} className={'px-4 py-4 ' + (i ? 'border-l border-white/10' : '')}><p className="font-mono text-lg sm:text-xl">{value}</p><p className="mt-1 text-[9px] uppercase tracking-[.14em] text-white/38">{label}</p></div>)}
            </div>
          </div>
        </div>
      </section>

      <section id="market" className="mx-auto max-w-[1480px] px-5 py-20 lg:px-10 lg:py-28">
        <div className="mb-10 flex flex-col justify-between gap-7 lg:flex-row lg:items-end">
          <div><p className="mb-3 flex items-center gap-2 text-[10px] uppercase tracking-[.18em] text-[#c8ff30]"><span className="h-px w-7 bg-[#c8ff30]" />Live capacity</p><h2 className="text-4xl font-medium leading-[.95] tracking-[-.055em] sm:text-6xl">Find your next<br /><span className="text-white/40">inference engine.</span></h2></div>
          <div className="w-full max-w-xl space-y-3">
            <div className="relative"><Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-white/35" /><Input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Search model, GPU, region, provider..." className="h-11 rounded-full border-white/12 bg-white/[.04] pl-10 placeholder:text-white/30" /></div>
            <div className="flex flex-wrap gap-2">{['All models', 'Language', 'Reasoning', 'Image'].map((item) => <button key={item} onClick={() => setFilter(item)} className={'rounded-full border px-3 py-1.5 text-[10px] uppercase tracking-[.11em] transition-colors ' + (item === filter ? 'border-[#c8ff30] bg-[#c8ff30] text-black' : 'border-white/12 text-white/48 hover:text-white')}>{item}</button>)}</div>
          </div>
        </div>
          ...
          {visible.map((p, i) => (
            <article key={p.id} className="compute-card group relative overflow-hidden rounded-[22px] border border-white/10 bg-[#0e100f] p-5 transition-all duration-500 hover:-translate-y-1 hover:border-white/20" style={{ '--provider-tint': p.tint } as React.CSSProperties}>
              ...
              <div className="relative flex items-start justify-between">
                ...
                <Badge variant="outline" className="border-white/10 text-[9px] text-white/50"><ShieldCheck data-icon="inline-start" className="text-[#c8ff30]" />Verified</Badge>
              </div>
        ...
        {!visible.length && <div className="rounded-2xl border border-dashed border-white/15 py-16 text-center text-sm text-white/45">No live capacity matches that search.</div>}
      </section>

      <section id="protocol" className="border-y border-white/[.08] bg-[#0b0c0b]">
      ...
      {selected && <div className="fixed inset-0 z-[80] grid place-items-center bg-black/75 p-4 backdrop-blur-xl" role="dialog" aria-modal="true">
        <div className="w-full max-w-md overflow-hidden rounded-[24px] border border-white/12 bg-[#111311]">
          <div className="flex justify-between border-b border-white/10 p-5"><div><p className="text-[9px] uppercase tracking-[.15em] text-[#c8ff30]">Inference checkout</p><h2 className="mt-2 text-xl font-medium">{selected.model}</h2><p className="mt-1 text-xs text-white/42">via {selected.name}</p></div><button onClick={() => { setSelected(null); setRun('idle'); }} className="size-8 rounded-full border border-white/10 text-white/50">x</button></div>
          <div className="space-y-4 p-5">
            ...
            {run === 'done' ? <div className="rounded-2xl border border-[#c8ff30]/20 bg-[#c8ff30]/[.06] p-4"><p className="flex gap-2 text-sm font-medium text-[#c8ff30]"><Check className="size-4" />Inference verified</p><p className="mt-2 text-xs text-white/45">Receipt 0x7c4...9a2 is ready to settle.</p></div> : <p className="text-xs leading-5 text-white/40">Prototype only. This test will not spend funds.</p>}
